...
class PhishingQuizGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Phishing Awareness Quiz")
        ...
        self.score = 0
        self.user_answer = tk.IntVar(value=0)

        # Create GUI elements
        self.create_welcome_screen()

    def create_question_bank(self):
        # Same 100-question bank as in phishing_quiz.py
        return [
            {
            ...
            },
            {
                ...
                "answer": 1,
                "feedback": "Correct! Charge notification emails are often phishing—verify with the service provider.\nIncorrect. Charge notification emails can be phishing scams; verify directly."
            },
            {
    ...
    def create_welcome_screen(self):
        ...
        # Welcome message
        ttk.Label(welcome_frame, text="Welcome to the Phishing Awareness Quiz!", font=("Helvetica", 16, "bold")).pack(pady=10)
        ttk.Label(welcome_frame, text="You will be asked 10 questions about phishing and cybersecurity.\nSelect the correct answer (1 or 2) for each question.", font=("Helvetica", 12), wraplength=500, justify="center").pack(pady=10)

        # Start button
        ttk.Button(welcome_frame, text="Start Quiz", command=self.start_quiz, style="TButton").pack(pady=20)
    ...
    def start_quiz(self):
        ...
        self.selected_questions = random.sample(self.question_bank, 10)
        self.current_question_index = 0
        self.score = 0
    ...
    def display_question(self):
        ...
        ttk.Label(question_frame, text=f"Question {question_number}/10", font=("Helvetica", 14, "bold")).pack(pady=5)
        ttk.Label(question_frame, text=question["text"], font=("Helvetica", 12), wraplength=500, justify="center").pack(pady=10)

        # Answer options
        ttk.Radiobutton(question_frame, text="1", variable=self.user_answer, value=1, style="TRadiobutton").pack(anchor="w", padx=20)
        ttk.Radiobutton(question_frame, text="2", variable=self.user_answer, value=2, style="TRadiobutton").pack(anchor="w", padx=20)

        # Submit button
        ttk.Button(question_frame, text="Submit", command=self.submit_answer, style="TButton").pack(pady=20)

        # Feedback label (initially empty)
        self.feedback_label = ttk.Label(question_frame, text="", font=("Helvetica", 11), wraplength=500, justify="center", foreground="#555555")
        self.feedback_label.pack(pady=10)

        # Next button (initially hidden)
        self.next_button = ttk.Button(question_frame, text="Next", command=self.next_question, style="TButton")
    ...
    def submit_answer(self):
        if self.user_answer.get() not in [1, 2]:
            messagebox.showwarning("Input Error", "Please select an answer (1 or 2).")
            return
        ...
        if user_answer == question["answer"]:
            self.score += 1
            self.feedback_label.config(text=feedback[0], foreground="#388e3c")
        else:
            self.feedback_label.config(text=feedback[1], foreground="#d32f2f")

        # Disable radio buttons and submit button
        for widget in self.root.winfo_children():
            for child in widget.winfo_children():
                if isinstance(child, (ttk.Radiobutton, ttk.Button)) and child["text"] != "Next":
    ...
    def show_final_score(self):
        ...
        # Display score
        ttk.Label(score_frame, text="Quiz Complete!", font=("Helvetica", 16, "bold")).pack(pady=10)
        ttk.Label(score_frame, text=f"Your score: {self.score}/10", font=("Helvetica", 14)).pack(pady=10)

        # Display summary and tips
        if self.score >= 9:
            summary = "Excellent! You’re a phishing detection expert."
            tip = "Keep practicing by staying vigilant and following best practices."
        elif self.score >= 6:
            summary = "Good job! You have a solid understanding of phishing."
            tip = "Tip: Always double-check email addresses and avoid clicking links in unsolicited emails."
        elif self.score >= 3:
            summary = "You’re on the right track, but there’s room for improvement."
            tip = "Tip: Review the awareness guide (docs/phishing_awareness.md) to learn more about phishing signs."
        else:
            summary = "You might be at risk of phishing attacks."
            tip = "Tip: Study the awareness guide (docs/phishing_awareness.md) and practice spotting phishing emails with simulate_phishing.py."

        ttk.Label(score_frame, text=summary, font=("Helvetica", 12), wraplength=500, justify="center").pack(pady=5)
        ttk.Label(score_frame, text=tip, font=("Helvetica", 11), wraplength=500, justify="center").pack(pady=5)

        # Restart button
        ttk.Button(score_frame, text="Restart Quiz", command=self.create_welcome_screen, style="TButton").pack(pady=20)

if __name__ == "__main__":
    root = tk.Tk()
    app = PhishingQuizGUI(root)
    root.mainloop()